...
data_Halpha_flux = clean_flux(fits.open('./NGC1275_lowres_3_sinc_Halpha_Flux.fits')[0].data)

data_NII = clean_flux(fits.open('./NGC1275_lowres_3_sinc_NII6583_Amplitude.fits')[0].data)
data_SN3_continuum = fits.open('./NGC1275_lowres_3_sinc_continuum.fits')[0].data
...
ax.imshow(image_data, cmap='viridis', origin='lower', vmin=np.nanpercentile(image_data, 5), vmax=np.nanpercentile(image_data, 99.5))  # Display the FITS image data as grayscale
...
def update_scatter_plot(selected_plots):
    ...
    layout = go.Layout(
        ...
        xaxis=dict(title='Log(<i>[NII]</i>/Hα)', tickformat='latex', showgrid=False),
        yaxis=dict(title='W<sub>Hα</sub> [Å]', tickformat='latex', type='log', showgrid=False),
        ...
        shapes=[
            ...
            {'type': 'line', 'x0': -1, 'x1': 1, 'y0': 0.5, 'y1': 0.5, 'line': {'color': 'black', 'width': 1, 'dash': 'dash'}},
            # Sloped dashed line
            {'type': 'line', 'x0': -1, 'x1': 0.75, 'y0': np.exp(-2.35 * -1 + np.log(0.5)), 'y1': np.exp(-2.35 * 0.75 + np.log(0.5)), 'line': {'color': 'black', 'width': 1, 'dash': 'dash'}}
        ],
        annotations=[
